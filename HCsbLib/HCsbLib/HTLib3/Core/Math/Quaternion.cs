...
namespace HTLib3
{
	...
	public struct Quaternion : IEquatable<Quaternion>, ISerializable
	{
		...
		public Quaternion(double[] axis, double angle)
		{
			...
			Vector axisNormalized = axis.HClone();
		...
		public Matrix RotationMatrix
		{
			...
			{
				if(RotationAngle == 0)
					return Matrix.IdentityMatrix(3);
		...
		public static Quaternion GetQuaternion(Matrix mat, double scale)
		{
			...
			Debug.Assert(mat.ColSize == 3, mat.RowSize == 3);
			double q0 = Math.Sqrt((mat[0,0] + mat[1,1] + mat[2,2] + 4)/4);
			Debug.Assert(double.IsNaN(q0) == false);
			Debug.Assert(q0 != 0);
			double q1 = (mat[2,1] - mat[1,2]) / (4*q0);
			double q2 = (mat[0,2] - mat[2,0]) / (4*q0);
			double q3 = (mat[1,0] - mat[0,1]) / (4*q0);
			Debug.AssertTolerant(0.001, (mat[0,0]+mat[1,1]) - 2*(q0*q0-q3*q3));
			Debug.AssertTolerant(0.001, (mat[0,0]+mat[2,2]) - 2*(q0*q0-q2*q2));
			Debug.AssertTolerant(0.001, (mat[1,1]+mat[2,2]) - 2*(q0*q0-q1*q1));
			Debug.AssertTolerant(0.001, (mat[1,0]+mat[0,1]) - (4*q1*q2)      );
			Debug.AssertTolerant(0.001, (mat[1,0]-mat[0,1]) - (4*q0*q3)      );
			Debug.AssertTolerant(0.001, (mat[0,2]+mat[2,0]) - (4*q1*q3)      );
			Debug.AssertTolerant(0.001, (mat[0,2]-mat[2,0]) - (4*q0*q2)      );
			Debug.AssertTolerant(0.001, (mat[2,1]+mat[1,2]) - (4*q2*q3)      );
			Debug.AssertTolerant(0.001, (mat[2,1]-mat[1,2]) - (4*q0*q1)      );
			Quaternion quater = new Quaternion(q0, q1, q2, q3);
			if(Debug.IsDebuggerAttached)
			{
				Matrix rotmat = quater.RotationMatrix;
				const double tolerance = 0.001;
				Debug.Assert(Math.Abs(rotmat[0,0] - mat[0,0]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[0,1] - mat[0,1]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[0,2] - mat[0,2]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[1,0] - mat[1,0]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[1,1] - mat[1,1]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[1,2] - mat[1,2]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[2,0] - mat[2,0]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[2,1] - mat[2,1]) < tolerance);
				Debug.Assert(Math.Abs(rotmat[2,2] - mat[2,2]) < tolerance);
			}
			return quater;
			//////////////////////////////////////////////////////////////////////////
		...
		public static Quaternion Parse(string s)
		{
			...
			Debug.Assert(rijk.Count == 4);
			for(int i_=0; i_<4; i_++) rijk[i_] = rijk[i_].Trim();
			Debug.Assert(rijk[1].EndsWith("i"));
			Debug.Assert(rijk[2].EndsWith("j"));
			Debug.Assert(rijk[3].EndsWith("k"));
