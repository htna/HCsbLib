...
namespace HTLib3
{
    public partial class Graph
    {
        ...
        public List<Node> FindPathMST(Node from, IEnumerable<Node> tos, Func<List<Node>, List<Node>, int> selector)
        {
            ...
            HTLib2.Tree<Node> tree = BuildTreeMST(from, null);
            foreach(Node to in toset)
            {
                HTLib2.Tree.Node treeto = tree.FindNode(to);
                List<HTLib2.Tree.Node> treepath = tree.NodesFromRootTo(treeto);
                List<Node> path = tree.GetValue(treepath);
                if(path.Count == 0)
                    continue;
                Debug.Assert(path.First() == from, path.Last() == to);
        ...
        public List<Node> FindPathRec(Node from, IEnumerable<Node> tos, Func<List<Tuple<Edge, object>>, List<Tuple<Edge, object>>, int> selector)
        {
            ...
            {
                Debug.Assert(found.First().Item1.nodes.Contains(from));
                foundnodes.Add(from);
            }
            for(int i=1; i<found.Count; i++)
            {
                Node[] midnode = found[i-1].Item1.nodes.HSelectCommon(found[i].Item1.nodes);
                Debug.Assert(midnode.Length == 1);
