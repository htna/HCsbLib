...
namespace HTLib3
{
    public partial class Graph<NODE,EDGE>
    {
        public class Dijkstra
        {
            ...
            {
                return Graph.Dijkstra.GetPathNode(elems.HConvertType<Elem, Graph.Dijkstra.Elem>(), target).HConvertType<Graph.Node, Node>();
            }
            public static Edge[] GetPathEdge(Elem[] elems, Node target)
            {
                return Graph.Dijkstra.GetPathEdge(elems.HConvertType<Elem, Graph.Dijkstra.Elem>(), target).HConvertType<Graph.Edge, Edge>();
            }
            //////////////////////////////////////////////////////////////////////////////////
            /// 
            /// Template classes
        ...
        public class Kruskal
        {
            public static Graph<NODE,EDGE> BuildMST(Graph<NODE,EDGE> graph, Func<Edge, double> edgecost)
            {
                ...
                foreach(Graph.Node node in mst.GetValueNodes())
                {
                    ...
                    Debug.Verify(mst2.AddNode(nodeval) != null);
                }
                foreach(Graph.Edge edge in mst.GetValueEdges())
                {
                    EDGE edgeval = ((Edge)edge).value;
                    Node node0 = (Node)edge.nodes[0]; NODE nodeval0 = graph.GetValue(node0); Debug.Assert(mst2.GetNode(nodeval0) != null);
                    Node node1 = (Node)edge.nodes[1]; NODE nodeval1 = graph.GetValue(node1); Debug.Assert(mst2.GetNode(nodeval1) != null);
                    Debug.Verify(mst2.AddEdge(nodeval0, nodeval1, edgeval) != null);
                }
                return mst2;
            }
