...
namespace HTLib3
{
    public partial class Graph
    {
        public Edge FindEdge(Node node1, Node node2)
        {
            ...
            {
                Debug.Assert(node2.nexts.ContainsKey(node1) == false);
                return null;
            }
            Debug.Assert(node2.nexts.ContainsKey(node1));
            Debug.Assert(node1.nexts[node2] == node2.nexts[node1]);
        ...
        public Edge AddEdge(Node node1, Node node2, Func<int,Node,Node,Edge> edgebuilder)
        {
            ...
            if(node1.nexts.ContainsKey(node2)) { Debug.Assert(node2.nexts.ContainsKey(node1)); return null; }
            if(node2.nexts.ContainsKey(node1)) { Debug.Assert(node1.nexts.ContainsKey(node2)); return null; }
            int id = edges.Count;
