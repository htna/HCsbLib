...
namespace HTLib2.Bioinfo
{
	public partial class Topol
	{
		public class ListDouble
		{
			...
			readonly HashSet<string> DOUBLEs; // double bond key as (atom1 name, atom2 name)

			ListDouble(List<string> lines)
			{
				_lines = lines;
				DOUBLEs = new HashSet<string>();
				foreach(string line in lines)
				{
					string[] tokens = line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
					HDebug.Assert(tokens[0] == "DOUBLE");
					HDebug.Assert((tokens.Length-1) % 2 == 0);
					for(int i=1; i<tokens.Length; i+= 2)
					{
						string atom1_name = tokens[i+0];
						string atom2_name = tokens[i+1];
						DOUBLEs.Add(GetKey(atom1_name, atom2_name));
					}
				}
			}
			...
			public static ListDouble FromLines(List<string> lines)
			{
				return new ListDouble(lines);
			}
		}
	}
}
